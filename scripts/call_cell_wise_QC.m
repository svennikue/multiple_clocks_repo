%% Cell-wise QC: spikes, RPV, correlation-based dedup; save one MAT with all sessions
%% 19 Aug 2025
% loads your data,
% runs cell-wise QC per session (spikes, RPV, per-grid stability, and cell–cell correlations with correlation-based dedup),
% saves one master file qc_all_sessions.mat,
% saves a flat index of accepted cells accepted_cells_index.mat,
% saves a filtered dataset abcd_passed.mat that only contains accepted cells,
% and prints totals (how many cells passed / were excluded).
% Save this whole thing as cell_wise_QC.m and run it. No other files required.
...
min_cells_to_pass = 1;                     % a session is "usable" if >= this many units pass
subject_list = 1:length(abcd_data.abcd_data);

% ---- QC settings that will be recorded into qc_all.meta ----
qc_settings.MinSpikes       = 300;     % base inclusion
qc_settings.RefracMS        = 1.5;     % refractory window for RPV
qc_settings.RPV_Frac_Thresh = 0.01;    % <1% ISIs in refractory
qc_settings.BinSizeS        = 0.10;    % 100 ms bins for correlations
qc_settings.CorrWindow      = 'task';  % 'task' or 'overlap'
qc_settings.CorrThresh      = 0.50;    % high corr threshold for dedup
qc_settings.LowFRFrac       = 0.20;    % 20% of mean per-grid (stability metric only)
qc_settings.MinTrialDur     = 0.050;   % 50 ms min trial length
...
qc_all.meta = struct( ...
    ...
    'qc_function', 'local: qc_single_session (in this file)' ...
    );
...
for sub = subject_list
    ...
    qc = qc_single_session(subj, ...
        ...
        'MinSpikes',       qc_settings.MinSpikes, ...
        'RefracMS',        qc_settings.RefracMS, ...
        'RPV_Frac_Thresh', qc_settings.RPV_Frac_Thresh, ...
        'BinSizeS',        qc_settings.BinSizeS, ...
        'CorrWindow',      qc_settings.CorrWindow, ...
        'CorrThresh',      qc_settings.CorrThresh, ...
        'LowFRFrac',       qc_settings.LowFRFrac, ...
        'MinTrialDur',     qc_settings.MinTrialDur);
    ...
    qc_all.sessions(sub).qc            = qc;           % full per-cell QC struct (includes high-corr dedup info)
end
...
% Simple per-session overview table
...
% ---- Flat list of accepted cells across all sessions ----
accepted = struct('session',{},'subject_id',{},'cell_idx',{},'electrodeLabel',{},'regionLabel',{});
k = 0;
for sub = subject_list
    sess = qc_all.sessions(sub);
    if isempty(sess.pass_idx), continue; end
    for ci = sess.pass_idx(:)'   % original cell indices (into subj.neural_data)
        k = k + 1;
...
accepted_mat = fullfile(deriv_dir, 'accepted_cells_index.mat');
save(accepted_mat, 'accepted', '-v7');

% ---- Filter original dataset to only accepted cells (keeps structure) ----
abcd_passed = abcd_data;  % shallow copy
for sub = subject_list
    keep = qc_all.sessions(sub).pass_mask;
    if isempty(keep)
        abcd_passed.abcd_data(sub).neural_data = [];
    else
...
passed_mat = fullfile(deriv_dir, 'abcd_passed.mat');
save(passed_mat, 'abcd_passed', '-v7.3');   % -v7.3 for large structs

% ---- Print final tally ----
fprintf('\nSaved QC for %d sessions to:\n  %s\n', numel(subject_list), one_mat);
fprintf('Also saved:\n  %s  (flat index of accepted cells)\n  %s  (dataset filtered to accepted cells)\n', ...
    accepted_mat, passed_mat);
fprintf('Total cells: %d | Passed: %d (%.1f%%) | Excluded: %d\n\n', ...
    total_cells, total_pass, 100*total_pass/max(1,total_cells), total_fail);
...
% Computes, per cell:
%   (1) n_spikes
%   (2) RPV (ISI < RefracMS ms)
%   (3) Per-grid FR stability (mean, CV, lowFR fraction)
%   (4) Cell–cell zero-lag correlations (binned counts) over chosen window
% Accept/Reject:
%   - Base acceptance: spikes >= MinSpikes AND RPV_frac < RPV_Frac_Thresh
%   - High-corr dedup: among base-accepted cells, any corr >= CorrThresh
%     → keep the "better" one: more spikes, lower RPV, lower lowFR fraction,
%       lower FR CV, higher overall FR (as final tie-breaker).
%
% Returns:
%   qc (struct array) with fields: electrodeLabel, regionLabel, n_spikes,
%   overall_FR_Hz, RPV_frac, RPV_count, grid_FR_mean, grid_FR_CV,
%   lowFR_grid_frac, corr_max, corr_max_partner_idx, base_accept,
...
% Grid list for stability measures
...
for c = 1:nC
    ...
    % Save now; fill correlation later
    qc(c).n_spikes         = nsp;
    ...
    qc(c).is_reliable      = base_ok;     % temporary; dedup may demote
    qc(c).fail_reasons     = {};
    if ~base_ok
        if nsp < MinSpikes
...
switch lower(p.Results.CorrWindow)
    ...
    case 'overlap'
        % overlap across base-accepted cells; fall back to task if empty
...
if numel(acc_idx) >= 2 && ~isempty(R)
    ...
    for a = 1:numel(acc_idx)
        ...
        [~, ord] = sortrows(rankMat);     % ascending on each column
        winner_local = ord(1);
