...
for repeat in range(len(behaviour_all)):
    ...
    if curr_task in [10,11,12, 21, 22]:
        continue
...
for repeat in range(len(behaviour_all)):
    ...
    curr_repeat = behaviour_all[repeat, 0]
    if curr_repeat == 0:
        lowest = behaviour_all[repeat, 10] # task onset
        middle_low = behaviour_all[repeat, 4] # found first D
    if curr_repeat == 1 and behaviour_all[repeat, 0] < 1:
        middle_high = behaviour_all[repeat, 4] # found first D
    if curr_repeat == 9:
        highest = behaviour_all[repeat, 4] # finished task
