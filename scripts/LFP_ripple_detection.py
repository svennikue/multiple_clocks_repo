...
for task_to_check in range(1, 23):  
# for task_to_check in range(1,4):
    if task_to_check in [10, 11, 12]: 
        continue
    if analysis_type == 'exploration_trials':
        ...
        for i in range(1, len(behaviour_all)):
            if i == 1: 
                ...
                task_config.append([behaviour_all[i-1, 5], behaviour_all[i, 6],behaviour_all[i, 7],behaviour_all[i, 8]])
    ...
    for task in range(index_lower, index_upper):
        ...
        if plotting_distr == True:
            if analysis_type == 'grid_wise' and task == index_lower:
                ...
                plt.title(f"Ripple frequency when solving grid {task_to_check} [10 correct repeats]")
