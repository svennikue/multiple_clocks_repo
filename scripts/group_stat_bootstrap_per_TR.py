...
def _tstat_1d(v):
    """One-sample t of `v` (a 1-D vector) against zero. Returns 0 if degenerate."""
    m = np.mean(v); s = np.std(v)
    if s == 0:
        return 0.0
    return m / s * np.sqrt(len(v))
...
def write_paper_summary(output_dir, root, dir_glob, tr_order, n_subj, n_perm,
    ...
    for model, recs in results_by_model.items():
        ...
        if n_all == 0:
            L("_No clusters._\n")
            continue
        L("| # | cluster_id | peak ROI | mass | p_FWE | peak MNI (x, y, z) | "
          "peak TR | peak t | n_vox 4-D | n_vox spatial | TRs spanned | sig |")
        L("|---|---|---|---|---|---|---|---|---|---|---|---|")
...
def _run_svc_and_roimean(model, data, tr_order, roi_mask, mask_name,
    ...
    return recs, (obs_clusters, obs_masses, p_fwe_per_cl, roi_ts, t_vec)
...
def run(root, output_dir, n_perm=10, p_thres=0.001, n_jobs=-1, models=MODELS,
    ...
    svc_output_by_model = {}   # (mask_name, model) -> summary for the .md
    ...
    for model in models:
        ...
        for mask_name, roi_mask in svc_masks.items():
            ...
            svc_recs, roimean = _run_svc_and_roimean(
            ...
            svc_output_by_model[(mask_name, model)] = {
                "svc4d_clusters": svc_recs,
                "roimean_1d_clusters": [
                    {"tr_start": tr_order[a], "tr_end": tr_order[b - 1],
                     "cluster_mass": float(m), "p_FWE": p}
                    for (a, b), m, p in zip(roimean[0], roimean[1], roimean[2])
                ],
            }
