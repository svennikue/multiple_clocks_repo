...
def prep_and_model_human_cells(data_dict):
    ...
    for sub in data_dict:
        ...
        for grid_idx, grid_config in enumerate(data_dict[sub]['reward_configs']):
            ...
            # (1 bin = 50ms)
            # timings task has to be start A - find A - find B - find C - find D
            ...
            # add a column in the start which is a copy of the last column, but moved down by one. 
            # Copy the 4th column (index 3) and shift down
            start_A = np.zeros_like(timings_task[:, 0])  # Create a zero array of the same size as timings
            start_A[1:] = timings_task[:-1, 3]  # Shift the 4th column down by one
            
            # Insert the new column as the first column in the array
            timings_task = np.insert(timings_task, 0, start_A, axis=1)
            ...
            for repeat in range(0, len(timings_task)):
                ...
                prep_repeat_dict = mc.analyse.helpers_human_cells.prep_behaviour_one_repeat(timings_repeat, first_bin, locations_curr_grid)


                
                # then model per repeat/grid/subject
                modelled_data[sub][grid_idx][repeat] = mc.simulation.predictions.set_continous_models_ephys(prep_repeat_dict)
...
def prep_behaviour_one_repeat(timings_repeat, t_first_bin, locations_curr_grid):
    ...
    subpath_locs = [locations_curr_grid[t_first_bin:timings_repeat[1]], 
                    locations_curr_grid[timings_repeat[0]:timings_repeat[1]], 
