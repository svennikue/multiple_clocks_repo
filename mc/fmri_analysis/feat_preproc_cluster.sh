...
for subjectTag in "${subjects[@]}"; do
    ...
    cat $analysisDir/templates/preproc.fsf | sed "s/1246648320/${numVoxels}/g" |sed "s/1670/${numVols}/g" | sed "s/sub-01/sub-$subjectTag/g" | sed "s:/vols/Scratch/xpsy1114:${scratchDir}:g" > $funcDir/sub-${subjectTag}_1_design_preproc.fsf
    echo Now starting first feat preproc
    feat $funcDir/sub-${subjectTag}_${folder}_1_design_preproc.fsf
    ...
    # really important to here take the respective .fsf file of the sequence I prepared manually! Different TE, TR, echo spacing
    cat $analysisDir/templates/preproc.fsf | sed "s/1246648320/${numVoxels}/g" |sed "s/1670/${numVols}/g" | sed "s/sub-01/sub-$subjectTag/g" | sed "s:/vols/Scratch/xpsy1114:${scratchDir}:g" > $funcDir/sub-${subjectTag}_2_design_preproc.fsf
