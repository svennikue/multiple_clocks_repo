    ...
    for task_half in 01 02; do
        ...
        for currEV in ${featstatsDir}/pe*.nii.gz; do
            ...
            if [[ "$number" =~ ^[0-9]+$ ]] && (( number >= 0 && number <= ${max_no_EVs} )); then
                ...
                # bbregister --s sub-14 --mov ~/scratch/data/derivatives/sub-14/func/preproc_clean_01.feat/example_func.nii.gz --reg ~/scratch/data/derivatives/sub-14/func/preproc_clean_01.feat/reg/freesurfer/register.dat --bold
...
# another way: first, transform your files to cifti. then use wb_command -volume-to-surface-mapping and map to surface
# The ribbon mapping method constructs a polyhedron from the vertex's neighbors on each surface, and estimates the amount 
# of this polyhedron's volume that falls inside any nearby voxels, to use as the weights for sampling.
# or mri_vol2surf